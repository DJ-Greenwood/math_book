...
\begin{itemize}
    ...
    By combining these two parts, we capture how both functions contribute to the rate of change of their product. This approach avoids treating \( g(x) \) and \( h(x) \) as independent since they both vary with \( x \).
    
    \subsubsection{Example}
    
    If \( f(x) = x^2 \cdot \sin(x) \):
    
    1. Set \( g(x) = x^2 \) and \( h(x) = \sin(x) \).
    2. Compute \( g'(x) = 2x \) and \( h'(x) = \cos(x) \).
    ...
    This gives the derivative of the product \( f(x) = x^2 \cdot \sin(x) \) as:
